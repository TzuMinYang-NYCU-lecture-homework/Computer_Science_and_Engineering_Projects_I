...
ceiling = cylinder(pos = vector(0, 0, -0.5), axis = vector(0, 0, 1), radius = 0.02, color = color.red)
...
ball_s = sphere(pos = vector(1, 0 ,0), radius = 0.05, color = color.yellow)

string_m = cylinder(pos = vector(0, 0, 0), radius = 0.003, length = 1, color = color.white)
ball_m = sphere(pos = vector(1, 0 ,0), radius = 0.05, color = color.white)
...
def Command(data):
    if data!= None:
        pendulum_rotate(data[0])

#df = 'Command'
def setup():
    profile = {
        'dm_name':'ConicalPendulum1',
        'odf_list':[Command]
    }
    dai(profile)
