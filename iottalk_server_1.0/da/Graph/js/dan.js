var dan = (function () {
    ...
    function get_alias_name(callback) {
        var all_alias_name = [];
        var odf_selected_count = 0;
        function get_alias_name_callback(data, error) {
            all_alias_name.push(data);
            if(all_alias_name.length == odf_selected_count)
                callback(all_alias_name);
        }
        for (var i = 0; i < _df_list.length; i++) {
            var _df_name = _df_list[i];
            if (_df_is_odf[_df_name] && _df_selected[_df_name]) {
                odf_selected_count++;
                csmapi.get_alias_name(_mac_addr, _df_name, get_alias_name_callback);
            }
        }
