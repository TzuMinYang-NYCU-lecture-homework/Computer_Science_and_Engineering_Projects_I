...
def register_device(addr):
    ...
    for i in Target_df_list: TargetTimestamp[i] = ''
    ...
    if csmapi.register(MAC,profile):
        ...
        thx = 'disable' #Disable control channel
        if thx == None:
...
def pullTarget(FEATURE_NAME):
    global TargetTimestamp

    data = csmapi.pull(TargetMACaddr, FEATURE_NAME, 'http://' + TargetENDPOINT + ':9999')
        
    if data != [] and data != None:
        if TargetTimestamp[FEATURE_NAME] == data[0][0]:
            return None
        TargetTimestamp[FEATURE_NAME] = data[0][0]
        if data[0][1] != []:
            return (data[0][0].split('.'))[0], data[0][1]
        else: return None
    else:
        return None
...
def pull_NCTUBus(URL, post_data):

    # if state == 'RESUME': data = csmapi.pull(MAC,FEATURE_NAME)
    # else: data = []
    data = csmapi.pull_NCTUBus(URL, post_data)
